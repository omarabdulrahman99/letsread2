...
class StatsChart extends Component {
    ...
    async componentDidMount() {
        const booksres = await axios.post("/api/usershelfbooks", {
            goodreadId: this.props.thisuser.goodreadId
        });
        const books = booksres.data.usershelfbooks;

        const shelfres = await axios.post("/api/shelflist", {
            user: this.props.thisuser
        });
        const shelves = shelfres.data.GoodreadsResponse.shelves[0].user_shelf;

        const profileres = await axios.post("/api/profileonly", {
            goodreadId: this.props.thisuser.goodreadId
        });
        const joindate =
            profileres.data.profileinfo.GoodreadsResponse.user[0].joined[0];
        ...
        this.setState(
            {
                ...
                joindate: joindate,
                draw3view: "year",
                totalreadChecked: "year"
            },
            () => {
                this.setState({ drawcharts: true });
            }
        );
    ...
    getNoteSecs = async () => {
        const notetimesecs = await axios.post("/api/getnotetimesecs", {
            user: this.props.thisuser
        });
    ...
    drawChart = () => {
        ...
        let data = [{ label: "useless label", values: pages }];
    ...
    drawChart2 = () => {
        ...
        var top6 =
            shelves.length >= 6 ? shelves.slice(shelves.length - 6) : shelves;
    ...
    drawChart3 = () => {
        ...
        if (this.state.draw3view === "week") {
            ...
            for (let i = 0; i < data[0].values.length; i++) {
                data[0].values[i].x = dateFns.format(
                    data[0].values[i].x,
                    "MMM D"
                );
            }
            ...
            return (
                <div>
                    <div className="chartTitle">
                        Books read by this Week/Month/Year/All-time
                    </div>
                    <LineChart
                        data={data}
                        width={width || 620}
        ...
        if (this.state.draw3view === "all") {
            ...
            let joindateparsed = dateFns.parse(
                this.state.joindate.slice(this.state.joindate.indexOf("/") + 1)
            );
            let joindate = dateFns.format(joindateparsed, "YYYY");
            let currDateYear = dateFns.format(currDate, "YYYY");
            let numdiffyears =
                dateFns.differenceInYears(currDateYear, joindate) + 1;
            let years = [];

            for (let i = 0; i < numdiffyears; i++) {
                let tempyear = dateFns.format(
                    dateFns.subYears(currDateYear, i),
                    "YYYY"
                );
            ...
            for (let i = 0; i < (booksread ? booksread.length : 0); i++) {
                let tempbook = dateFns.format(
                    booksread[i].date_updated[0],
                    "YYYY"
                );
            ...
            data[0].values.unshift({
                x: dateFns.format(
                    dateFns.subYears(data[0].values[0].x, 1),
                    "YYYY"
                ),
                y: 0
            });
    ...
    totalreadChartRadioChange = e => {
        ...
        this.setState({
            totalreadChecked: checkedname,
            draw3view: checkedname
        });
    };

    render() {
        var drawcharts = this.state
            ? this.state.drawcharts
                ? this.state.drawcharts
                : false
            : false;
            ...
            <div>
                {drawcharts ? (
                    <div className="chartLayout">
                        ...
                        <div className="pagesChart">{this.drawChart()}</div>

                        <div className="shelvesChart">{this.drawChart2()}</div>
                        ...
                        <div className="totalreadChart">
                            ...
                            <div className="totalreadChartRadioGroup">
                                <input
                                    ...
                                    checked={
                                        this.state.totalreadChecked === "week"
                                    }
                                    value="week"
                                    onChange={this.totalreadChartRadioChange}
                                />
                                ...
                                <input
                                    ...
                                    className="chartRadio"
                                    checked={
                                        this.state.totalreadChecked === "month"
                                    }
                                    value="month"
                                    onChange={this.totalreadChartRadioChange}
                                />
                                ...
                                <input
                                    ...
                                    className="chartRadio"
                                    checked={
                                        this.state.totalreadChecked === "year"
                                    }
                                    value="year"
                                    onChange={this.totalreadChartRadioChange}
                                />
                                ...
                                <input
                                    ...
                                    className="chartRadio"
                                    checked={
                                        this.state.totalreadChecked === "all"
                                    }
                                    value="all"
                                    onChange={this.totalreadChartRadioChange}
                                />
