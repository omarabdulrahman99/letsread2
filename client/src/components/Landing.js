...
class Landing extends Component {
	...
	async handleSubmit(e) {
		...
		if (event.nodeName === "BUTTON") {
			...
			this.setState({
				qresults: dataresults.data,
				shelflist: shelflist,
				usershelfbooksraw: usershelfbooksraw,
				searchpage: true
			});
	...
	renderContent() {
		if (this.state.searchpage !== true) {
			...
			return (
				<SearchResults
					tdata={this.state.qresults}
					shelflist={this.state.shelflist}
					usershelfbooksraw={this.state.usershelfbooksraw}
				/>
			);
		}
	}
	...
	render() {
		return <div className="centeredsearch">{this.renderContent()}</div>;
	}
}
