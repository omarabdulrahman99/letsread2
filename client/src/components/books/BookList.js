...
class BookList extends Component {
    ...
    async mybooksload(user) {
        if (!user) {
            ...
            const booksres = await axios.post("/api/mybooksload", {
    ...
    leftnav() {
        ...
        return (
            <ul>
                <div className="goodReadsLogo" />
                <hr className="goodReadsLogoHR" />
    ...
    async tableload(e) {
        ...
        this.setState({ shelf: shelf, grid2: "grid2" }, async () => {
            const booklistres = await axios.post("/api/selectshelf", {
                shelfname: this.state.shelf,
                goodreadId: this.props.auth.goodreadId
            });
    ...
    tablerender() {
        ...
        if (
            ...
            rows = [
                { cover: "", title: "", author: "", avgrating: "", year: "" }
            ];
        } else {
    ...
    async previewState(e) {
        ...
        var bookDates = await axios.post("/api/booksched", {
            action: "get",
            user: this.props.auth,
            bookid: this.bookpreviewId
        });
        ...
        return (
            <aside className="previewnote style-13">
                <div className="bookprevinfo">
                    <img src={cover} alt="bookpreviewcover" />
                ...
                <div className="noteprevinfo">
                    <a
                        className="toBkNoteLink"
                        onClick={e => {
                            e.preventDefault();
                            this.setState({ booknote: true });
                        }}
                    >
                        Go to book notes
                    </a>
