...
class BookNote extends Component {
    ...
    saveTime = async secscount => {
        const timesaved = await axios.post("/api/savenotetime", {
    ...
    saveEditorToDB = async () => {
        ...
        const res = await axios.post("/api/saveChapEditor", {
            user: this.props.user,
            bookid: this.props.bookid,
            chapnum: this.state.chapnum,
            editor: rawString
        });
    ...
    addChapter = async e => {
        ...
        var valid =
            patt.test(this.state.chapnum) &&
            (parseInt(this.state.chapnum) >= 1 &&
                parseInt(this.state.chapnum) <= 170);
        var valid2 = true;
    ...
    addBookNote = e => {
        ...
        if (predictions) {
            let editorPredictions = EditorState.createWithContent(
                convertFromRaw(JSON.parse(predictions))
            );
    ...
    renderEditCards = () => {
        ...
        sortedchaps = this.state.chapters.sort(function(a, b) {
            ...
            if (this.state.cardselects[chapnum] === "selected") {
                cardsarr.push(
                    <div
                        ...
                        id={this.state.chapters[i].chapnum}
                        onClick={this.editCardClick}
                        className="bkcardeditselect"
                    >
                        <div className="cardTitle">
                            Chapter {this.state.chapters[i].chapnum}
                        </div>
                        <hr />
                        <input
                            id={this.state.chapters[i].chapnum}
                            type="text"
                            placeholder={this.state.chapters[i].title}
                            style={inputstyle}
                            className="cardTitle2"
                            value={
                                this.state.cardinputs[
                                    this.state.chapters[i].chapnum
                                ]
                            }
    ...
    cardonChange = e => {
        ...
        this.setState(prevState => ({
            cardinputs: { ...prevState.cardinputs, [cardid]: value }
        }));
    };
    ...
    turnjsSwitch = async e => {
        ...
        var profileres = await axios.post("/api/profileonly", {
            goodreadId: this.props.user.goodreadId
        });
        var profileuserdata =
            profileres.data.profileinfo.GoodreadsResponse.user;

        let turnjsEditor = predictions
        ...
        if (predObj !== "Empty") {
            ...
            for (var i = 0; i < turnjstext.length; i++) {
                ...
                for (var j = 0; j < predObj.blocks.length; j++) {
                    for (
                        var k = 0;
                        k < predObj.blocks[j].inlineStyleRanges.length;
                        k++
                    ) {
                        var start =
                            predObj.blocks[j].inlineStyleRanges[k].offset +
                            blockcount;
                        var end =
                            predObj.blocks[j].inlineStyleRanges[k].length +
                            start;
                        var style =
                            predObj.blocks[j].inlineStyleRanges[k].style;
            ...
            for (var m = 0; m < stylechars.length; m++) {
                if (
                    ...
                    let type = predObj.blocks[blocknum]
                        ? predObj.blocks[blocknum].type
                        : null;
                    ...
                    switch (type) {
                        case "header-one": {
                            if (linecount + 3 >= 17) {
                                divarr.push(
                                    <div key={m} style={stylechars[m][1]}>
                                        {stylechars[m][0]}
                                    </div>
                                );
                                pages[pagenum].push(
                        ...
                        case "unordered-list-item": {
                            ...
                            if (totallines >= 17) {
                                ...
                                if (olchars <= nextindex) {
                                    ...
                                    let divarrcut = divarr.splice(
                                        divarr.length - nextindex,
                                        nextindex
                                    );
                                    divarr2.length = 0;
                        ...
                        case "blockquote": {
                            ...
                            if (totallines >= 17) {
                                ...
                                if (olchars <= nextindex) {
                                    ...
                                    pages[pagenum].push(
                                        <div key={m}>
                                            <blockquote className="grapefruit">
                                                {divarr}
                                            </blockquote>
                                        </div>
                                    );
            ...
            this.setState(
                {
                    turnjstext: pages,
                    coverinfo: { chapnum: chapnum, title: title },
            ...
            this.setState(
                {
                    turnjstext: turnjstext,
                    coverinfo: { chapnum: index, title: title },
                    profile: profileuserdata
                },
                () => {
                    this.setState({ turnjs: "on" });
    ...
    trashButton = () => {
        return (
            <div className="icon-trash" onClick={this.delChapters}>
                ...
                <div className="trash-line-2" />
                <div className="trash-line-3" />
            </div>
        );
    };
    ...
    saveedits = async () => {
        ...
        var newchapters = await axios.post("/api/updatechapters", {
            user: user,
            bookid: bookid,
            chapters: chapters
        });
        ...
        this.setState(
            { chapters: newchapters.data.chapters, savemsg: true },
            () => {
                setTimeout(() => {
    ...
    delChapters = async () => {
        ...
        var newchapters = await axios.post("/api/deletechapters", {
            user: this.props.user,
            bookid: this.props.bookid,
    ...
    firstLoad = async () => {
        const datares = await axios.post("/api/booksched", {
            user: this.props.user,
            bookid: this.props.bookid,
            action: "get"
        });
    ...
    render() {
        var warning1 = (
            <div className="bknotewarningred">
                Number must be between 1 and 170
            </div>
        );
        var warning2 = (
            <div className="bknotewarningred">
            ...
            <main className="cero">
                ...
                {this.state.turnjs === "on" ? (
                    <Turn
                        ...
                        coverinfo={this.state.coverinfo}
                    />
                ) : (
                    <div className="cardscontainer">
                        <div className="editcontainer">
                            <a href="/mybooks" className="editFlexBack">
                                Go Back
                            </a>
                            <div className="editflex">
                                <div
                                    className="editFlexBack"
                                    onClick={this.seteditRender}
                                >
                                    Edit
                                </div>
                        ...
                        <div className="bkcards">
                            ...
                            >
                                ...
                                <ModalBody>
                                    <div>Predictions:</div>
                                    <DraftEditor
                                        initialEditor={this.state.initialEditor}
                                        saveEditorState={this.saveEditorState}
                                    />
                                    <button onClick={this.saveEditorToDB}>
                                        SAVE
                                    </button>
                                </ModalBody>
                                <ModalFooter />
                            ...
                            <Modal isOpen={this.state.addchapter} size="sm">
                                ...
                                <form>
                                    <ModalBody>
                                        <MDBInput
                                            id="chapnum"
                                            value={this.state.chapnum}
                                            onChange={e => {
                                                this.mStartOnChange(e);
                                            }}
                                            className="modalstartinput"
                            ...
                            {this.state.edit ? null : (
                                <div className="addCenter">
                                    <div
                                        className="addCenter"
                                        onClick={this.setChapterStart}
                                    >
                                        <div className="editFlexBack">
                                            Add Chapter
                                        </div>
                                    </div>
                                </div>
