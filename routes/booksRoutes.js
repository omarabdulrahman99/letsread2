...
module.exports = app => {
	app.get("/api/author/:author", async (req, res) => {
		const authres = await axios.get(
			`https://www.goodreads.com/api/author_url/${
				req.params.author
			}?key=${process.env.GOODREADS_KEY || keys.GOODREADS_KEY}`
		);
	...
	app.post("/api/shelflist", async (req, res) => {
		...
		const shelfres = await axios.get(
			`https://www.goodreads.com/shelf/list.xml?user_id=${gdrdId}&key=${process
				.env.GOODREADS_KEY || keys.GOODREADS_KEY}`
		);
	...
	app.get("/api/search/:queryall", async (req, res) => {
		...
		const searchres = await axios.get(
			`https://www.goodreads.com/search/index.xml?key=${process.env
				.GOODREADS_KEY || keys.GOODREADS_KEY}&q=${searchq}`
		);
	...
	app.post("/api/usershelfbooks", async (req, res) => {
		...
		const shelfbooksxml = await axios.get(
			`https://www.goodreads.com/review/list/?v=2&key=${process.env
				.GOODREADS_KEY || keys.GOODREADS_KEY}&id=${goodreadId}`
		);
		var jsonres = "";
	...
	app.post("/api/addtoshelf", async (req, res) => {
		...
		var shelfadded = await axios.post(
			`https://www.goodreads.com/shelf/add_to_shelf.xml?name=${name}&book_id=${book_id}&a=${a}`
		);
	...
	app.get("/api/friendrequests", async (req, res) => {
		var friendreqs = await axios.get(
	...
	app.post("/api/mybooksload", async (req, res) => {
		...
		const shelfres = await axios.get(
			`https://www.goodreads.com/shelf/list.xml?user_id=${
				req.body.goodreadId
			}&key=${process.env.GOODREADS_KEY || keys.GOODREADS_KEY}`
		);
		const shelfxml = shelfres.data;
	...
	app.post("/api/selectshelf", async (req, res) => {
		...
		const shelfbooksxml = await axios.get(
			`https://www.goodreads.com/review/list/?v=2&key=${process.env
				.GOODREADS_KEY ||
				keys.GOODREADS_KEY}&id=${goodreadId}&shelf=${shelfname}`
		);
		var jsonres = "";
	...
	app.post("/api/profileinfo", async (req, res) => {
		...
		var profilexml = await axios.get(
			`https://www.goodreads.com/user/show.xml?key=${process.env
				.GOODREADS_KEY || keys.GOODREADS_KEY}&id=${goodreadId}`
		);
		...
		var favbooksxml = await axios.get(
			`https://www.goodreads.com/review/list.xml?key=${process.env
				.GOODREADS_KEY ||
				keys.GOODREADS_KEY}&v=2&id=${goodreadId}&shelf=read&sort=rating&order=a`
		);
		var favbooksjson = "";
	...
	app.post("/api/booksched", async (req, res) => {
		...
		switch (action) {
			case "get":
				Book.findOne({ user: user._id, bookid: bookid }, function(
					err,
					book
				) {
					res.send({ dates: book });
			...
			case "save":
				...
				Book.findOneAndUpdate(
					{ bookid: bookid, user: user._id },
					{ $push: { datesched: datesched } },
			...
			case "edit":
				...
				Book.findOneAndUpdate(
					...
					{
						$set: {
							"datesched.$[elem].startdate": startdate,
							"datesched.$[elem].enddate": enddate
						}
					},
					{ new: true, arrayFilters: [{ "elem.titlenum": tnum }] },
					function(err, book) {
						if (err) {
						}
			...
			case "delete":
				...
				Book.findOneAndUpdate(
					{ bookid: bookid, user: user._id },
					{ $pull: { datesched: { titlenum: titlenum } } },
					{ new: true },
					function(err, book) {
						res.send({ book: book });
					}
				...
				Book.find({ datesched: { titlenum: titlenum } }, function(
					err,
					book
				) {});
	...
	app.post("/api/deletechapters", async (req, res) => {
		...
		Book.findOneAndUpdate(
			{ bookid: bookid, user: user },
			{ $pull: { chapters: { chapnum: { $in: chapnums } } } },
			{ new: true },
			function(err, book) {
				res.send({ book: book });
			}
	...
	app.post("/api/saveChapEditor", async (req, res) => {
		...
		Book.findOneAndUpdate(
			{ bookid: bookid, user: user },
			{ $set: { "chapters.$[elem].predictions": editor } },
			{ new: true, arrayFilters: [{ "elem.chapnum": chapnum }] },
			function(err, book) {
				res.send({ book: book });
			}
		);
